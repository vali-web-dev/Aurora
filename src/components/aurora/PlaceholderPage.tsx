      ...
      <div className="mx-auto max-w-3xl rounded-2xl border border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/70 p-8 shadow-sm">
        <nav aria-label="Breadcrumb" className="text-xs text-slate-500 dark:text-slate-400">
          <ol className="flex flex-wrap items-center gap-2">
            ...
            <li>
              <Link href={baseHref} className="hover:text-slate-700 dark:hover:text-slate-200">
                {baseTitle}
              </Link>
            </li>
            {segments.map((segment) => (
              <li key={segment.href} className="flex items-center gap-2">
                ...
                <Link href={segment.href} className="hover:text-slate-700 dark:hover:text-slate-200">
                  {segment.label}
                </Link>
              </li>
            ))}
        ...
        <div className="mt-6">
          <Link
            href={baseHref}
            className="inline-flex items-center rounded-lg border border-slate-200 dark:border-slate-800 bg-white/80 dark:bg-slate-900/70 px-4 py-2 text-sm font-medium text-slate-700 dark:text-slate-200 hover:border-slate-300 dark:hover:border-slate-700"
          >
            Back to {baseTitle}
          </Link>
        </div>
