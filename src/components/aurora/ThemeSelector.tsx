          ...
          <div className="grid grid-cols-2 md:grid-cols-4 gap-3">
            {modes.map((m) => (
              <button
                ...
                className={clsx(
                  'aurora-label group relative p-4 rounded-xl transition-all duration-300',
              ...
              >
                ...
                <p className="aurora-label text-xs font-semibold">{m.label}</p>
                {mode === m.value && (
          ...
          >
            <div className="text-left">
              <div className="flex items-center gap-2">
                <span className="aurora-label text-sm font-semibold">Aurora Veil</span>
                {isVeilEnabled && (
                  <span className="aurora-label text-[10px] font-semibold uppercase tracking-wide px-2 py-0.5 rounded-full bg-blue-600 text-white">
                    Active
                  </span>
                )}
              </div>
              <div className="aurora-label text-xs text-slate-500 dark:text-slate-400">90% fill + blur(5px) backdrop</div>
            </div>
            <span
              aria-hidden="true"
              className={clsx(
            ...
            {families.map((f) => (
              ...
              >
                ...
                />
                <p className="aurora-label text-xs font-semibold">{f.label}</p>
                <p className={clsx('aurora-label text-xs mt-1', family === f.value ? 'text-white/80' : 'text-slate-500 dark:text-slate-400')}>
                  {f.description}
                </p>
              </button>
            ))}
        ...
        >
          ...
          <div className="px-4 py-3 border-b border-slate-100 dark:border-slate-800 flex items-center justify-between">
            ...
            <button
              ...
              className="aurora-label text-slate-400 hover:text-slate-600 dark:hover:text-slate-300 transition-colors"
              aria-label="Close"
            >
              ✕
            </button>
          ...
          <div className="p-4 space-y-5">
              ...
              <div className="grid grid-cols-4 gap-2">
                {modes.map((m) => (
                  <button
                    ...
                    className={clsx(
                      'aurora-label py-2 px-2 rounded-lg transition-all duration-200 text-sm font-medium',
                      'flex flex-col items-center gap-1',
                      mode === m.value
                        ? 'aurora-label bg-blue-600 text-white shadow-lg ring-2 ring-blue-400 dark:ring-blue-500'
                  ...
                  >
                    ...
                    <span className="aurora-label text-xs">{m.label}</span>
                  </button>
                ))}
              </div>
            </div>
            ...
            <div className="space-y-2">
              ...
              >
                <span className="aurora-label flex items-center gap-2 text-sm font-medium">
                  Aurora Veil
                  {isVeilEnabled && (
                    <span className="aurora-label text-[10px] font-semibold uppercase tracking-wide px-2 py-0.5 rounded-full bg-blue-600 text-white">
                      Active
              ...
              <div className="grid grid-cols-5 gap-2">
                {families.map((f) => (
                  <button
                    ...
                    className={clsx(
                      'aurora-label py-2 px-1 rounded-lg transition-all duration-200',
                      'flex flex-col items-center gap-1 text-xs font-medium',
                      family === f.value
                        ? 'aurora-label text-white shadow-lg ring-2 ring-offset-2 dark:ring-offset-slate-900'
                        : 'aurora-label bg-slate-100 dark:bg-slate-800 text-slate-700 dark:text-slate-300 hover:bg-slate-200 dark:hover:bg-slate-700 border border-slate-200 dark:border-slate-700'
                    )}
                    style={
                      family === f.value
                        ? {
                  ...
                  >
                    ...
                    />
                    <span className={clsx('aurora-label', family === f.value ? 'text-white' : '')}>
                      {f.label[0]}
                    </span>
                  </button>
                ))}
